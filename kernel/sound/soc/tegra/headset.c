...
static void insert_headset(void)
{
    ...
    printk("%s: [seyoung] insert_headset\n", __func__);
    printk("%s: [seyoung] uart-%d, lineout-%d, hook-%d, jack-%d\n", __func__,             
            UART_enable,                  
            gpio_get_value(lineout_gpio), 
            gpio_get_value(HOOK_GPIO),    
...
{
    printk("%s: [seyoung] remove_headset\n", __func__);

    switch_set_state(&hs_data->sdev, NO_DEVICE);
    hs_data->debouncing_time = ktime_set(0, 100000000);  /* 100 ms */
    headset_alive = false;
    // tristate_uart(); /*seyoung*/
    // gpio_direction_output(UART_HEADPHONE_SWITCH, 0); /*seyoung*/
}
...
static void detection_work(struct work_struct *work)
{
    ...
    printk("%s: [seyoung] detection_work\n", __func__);
...
static void audio_dock_switch(void)
{
    ...
    if(!gpio_get_value(gpio_dock_in)){
        ...
        printk("[seyoung] HEADSET: gpio_dock_in is low\n");
    ...
    /* gpio_dock_in is high */
    }else{

        printk("[seyoung] HEADSET: gpio_dock_in is high\n");
...
static int __init headset_init(void)
{
    ...
    if (project_info == GROUPER_PROJECT_BACH){
        printk("[seyoung] HEADSET: gpio_dock_in PO5 \n");
        gpio_dock_in = TEGRA_GPIO_PO5;
    }
    else{
        printk("[seyoung] HEADSET: gpio_dock_in PU4\n");
        gpio_dock_in = TEGRA_GPIO_PU4;
    }
    if(project_info == GROUPER_PROJECT_BACH ||
            (project_info == GROUPER_PROJECT_NAKASI/* && pmic_id ==GROUPER_PMIC_TI*/)){
        printk(KERN_INFO "[seyoung] project_info is bach & nakasi");
        UART_enable = true;
    }
    else{
        printk(KERN_INFO "[seyoung] project_info is else");
    }
    ...
    //switch_config_gpio(); /* seyoung Config uart and headphone switch*/
