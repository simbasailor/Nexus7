...
#if 1 /* seyoung - Bluetooth Device Settings */
...
static noinline void __init grouper_setup_bluesleep(void)
{
    ...
    printk("[seyoung] grouper_setup_bluesleep\n");
...
#endif /* Bluetooth Device Settings*/
...
static void grouper_i2c_init(void)
{
    ...
    printk("[seyoung] grouper_i2c_init\n");
...
static struct platform_device *grouper_uart_devices[] __initdata = {
	//&tegra_uarta_device // added by seyoung
    &tegra_uartb_device,
	&tegra_uartc_device,
	&tegra_uartd_device,
	&tegra_uarte_device,
...
/* debug_uarts is ttyHS (0~4)  */
static struct platform_device *debug_uarts[] = {
	&debug_uarta_device, // deleted by seyoung
	&debug_uartb_device,
	&debug_uartc_device,
	&debug_uartd_device,
	&debug_uarte_device,
...
static void __init uart_debug_init(void)
{
    ...
    printk("[seyoung] uart_debug_init start\n");

	debug_port_id = get_tegra_uart_debug_port_id();

    printk("[seyoung] uart_debug_port_id is %d\n", debug_port_id);

	if (debug_port_id < 0) {  /* seyoung - If port id is lower than 0 or higher than max */
		debug_port_id = 3;
    ...
    printk("[seyoung] debug_port name is %s\n", uart_names[debug_port_id]);

	pr_info("Selecting %s as the debug port\n",
		uart_names[debug_port_id]);
	debug_uart_clk = clk_get_sys("serial8250.0",   /* debug uart clk setting */
				     uart_names[debug_port_id]);
	debug_uart = debug_uarts[debug_port_id];
	debug_uart_port_base = ((struct plat_serial8250_port *)(  /* debug uart mapbase setting */
			debug_uart->dev.platform_data))->mapbase;
	debug_uart_port_irq = ((struct plat_serial8250_port *)( /* debug uart irq setting */
			debug_uart->dev.platform_data))->irq;

    printk("[seyoung] debug_uart_port_base:%08x, debug_uart_port_irq:%08x \n", 
                                                debug_uart_port_base,
                                                debug_uart_port_irq);
	return;
}
...
static void __init grouper_uart_init(void)
{
    ...
    printk("[seyoung] grouper_uart_init\n");
    ...
    //tegra_uarta_device.dev.platform_data = &grouper_uart_pdata; /*seyoung - add uart a */
	...
	if (!is_tegra_debug_uartport_hs()) {
        ...
        printk("[seyoung] debug_uartport\n");
        uart_debug_init();
		/* Clock enable for the debug channel */
		if (!IS_ERR_OR_NULL(debug_uart_clk)) {    /* uart clk error check */
			pr_info("The debug console clock name is %s\n",
						debug_uart_clk->name);
			c = tegra_get_clock_by_name("pll_p");
            
            printk("[seyoung] debug uart parent clk is %d\n", c);
            ...
            printk("[seyoung] debug uart rate is %d\n", clk_get_rate(c));
        ...
        printk("[seyoung] debug_uartport is hs\n");
    }

    printk("[seyoung] tegra_serial_debug_init enter\n");

	/* seyoung - fiq serial console disable */
     tegra_serial_debug_init(debug_uart_port_base, debug_uart_port_irq,
				debug_uart_clk, -1, -1, false);
    

    /* seyoung - for ttyHS0~4 */
...
static void __init grouper_spi_init(void)
{
    ...
    printk("[seyoung] grouper_spi_init\n");
...
static void grouper_usb_init(void)
{
    ...
    printk("[seyoung] grouper_usb_init\n");
...
static void grouper_audio_init(void)
{
    ...
    printk("[seyoung] grouper_audio_init\n");
...
{
    printk("[seyoung] grouper_pinmux_init\n");
...
static void __init tegra_grouper_init(void)
{
	...
	grouper_setup_bluesleep();  /* seyoung - bluetooth BT_WAKEUP / BT_IRQ set */
